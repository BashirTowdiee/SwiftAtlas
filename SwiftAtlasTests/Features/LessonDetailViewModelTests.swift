...
struct LessonDetailViewModelTests {
  ...
  func successfulLoadPopulatesScreenModel() async {
    ...
    guard case .loaded(let model, let isStale) = viewModel.state else {
  ...
  func failedLoadUsesReadableAppError() async {
    let container = TestContainerFactory.makeContainer(
      lessonRepository: TestLessonRepository(
        lessonDetailError: AppError.network("Request timed out")),
  ...
  func refreshReplacesStateAfterRepositoryChange() async {
    ...
    let container = TestContainerFactory.makeContainer(
      ...
      exerciseRepository: TestExerciseRepository(exercises: [
        Exercise(id: 1, title: "One", isComplete: false, rationale: "R")
      ])
    )
    let viewModel = LessonDetailViewModel()
    ...
    guard case .loaded(let model, _) = viewModel.state else {
