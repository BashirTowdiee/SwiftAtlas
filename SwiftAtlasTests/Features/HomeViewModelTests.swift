...
struct HomeViewModelTests {
  ...
  func bindPopulatesDiagnosticsAndFlags() {
    ...
    let container = TestContainerFactory.makeContainer(
      appState: appState, featureFlagStore: featureFlags)
  ...
  func refreshReflectsContainerChanges() {
    ...
    #expect(viewModel.diagnostics?.reachability == .usingCachedData)
    #expect(
      viewModel.activeFlags == [
        .remoteSyncEnabled, .advancedDiagnosticsEnabled, .experimentalConcurrencyDemosEnabled,
      ])
  }
  ...
  func architectureHighlightsRemainStableAndNonEmpty() {
    ...
    #expect(
      viewModel.architectureHighlights.contains(
