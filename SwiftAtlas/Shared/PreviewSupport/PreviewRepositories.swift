...
struct PreviewLessonRepository: LessonRepository {
  ...
  func fetchLessonDetail(id: Lesson.ID, policy: CachePolicy) async throws -> LessonDetail {
    if case .failure(let error) = variant {
      throw error
    }
    ...
    return LessonDetail(
      ...
      comments: [
        LessonComment(
          id: 1, author: "Preview Mentor", body: "Treat previews as executable examples."),
        LessonComment(
          id: 2, author: "Preview Learner",
          body: "Show loading, empty, error, and dark mode states."),
      ],
