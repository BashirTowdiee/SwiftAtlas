...
enum FeatureFlag: String, CaseIterable, Identifiable, Codable, Sendable {
  ...
  var explanation: String {
    switch self {
    case .labsEnabled:
      String(localized: "Shows the teaching labs tab and the deeper language demos.")
    case .remoteSyncEnabled:
      String(localized: "Allows background refreshes against JSONPlaceholder.")
    case .advancedDiagnosticsEnabled:
      String(localized: "Shows richer cache and bundle diagnostics in Settings.")
    case .experimentalConcurrencyDemosEnabled:
      String(localized: "Enables cancellable and task-group concurrency demos.")
    }
  }
