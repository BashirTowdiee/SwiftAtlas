...
struct DefaultExerciseRepository: ExerciseRepository {
  ...
  func fetchExercises(for lessonID: Lesson.ID, policy: CachePolicy) async throws -> [Exercise] {
    ...
    case .cacheFirst, .staleWhileRevalidate:
      if let cached = try cacheStore.value(for: .exercises(lessonID), as: [Exercise].self),
        !cached.isEmpty
      {
        return cached
      }
    ...
    let exercises =
      ...
      .map {
        Exercise(
          ...
          rationale:
            "This exercise is deterministically mapped from the lesson id so the teaching app stays reproducible."
        )
      }
