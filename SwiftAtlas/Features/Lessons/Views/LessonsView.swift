                    ...
                    GSCard {
                        VStack(alignment: .leading, spacing: GSSpacing.small) {
                            GSSectionHeader(
                                ...
                                detail: "This screen loads JSONPlaceholder data, maps it into lesson models, caches it locally, then lets the view model apply filters and search."
                            )
                            GSInfoRow(title: "Sync", value: viewModel.lastSyncMessage)
                        }
                    }

                    GSAsyncStateView(state: viewModel.loadState, retry: {
