  ...
  var body: some View {
    VStack(alignment: .leading, spacing: GSSpacing.small) {
      GSSectionHeader(
        ...
        detail:
          "Exercises are deterministic fixtures mapped from the remote lesson id so the app stays predictable."
      )
        ...
        GSCard {
          VStack(alignment: .leading, spacing: GSSpacing.xSmall) {
            HStack {
              ...
              Spacer()
              GSBadge(
                title: exercise.isComplete ? "Done" : "Open",
                tone: exercise.isComplete ? .success : .warning)
            }
