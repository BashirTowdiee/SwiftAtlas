...
final class SettingsViewModel: ObservableObject {
    ...
    func reload() {
        ...
        do {
            ...
            secretStatus = String(format: String(localized: "Unable to read token: %@"), error.localizedDescription)
        }
    }
    ...
    func saveSecret() {
        ...
        do {
            ...
            secretStatus = String(format: String(localized: "Failed to save token: %@"), error.localizedDescription)
        }
    }
    ...
    func deleteSecret() {
        do {
            ...
            secretStatus = String(format: String(localized: "Failed to delete token: %@"), error.localizedDescription)
        }
    }
    ...
    func clearCache() {
        do {
            ...
            appState?.pushNotice(AppNotice(tone: .success, message: String(localized: "Cleared cached lesson data.")))
        } catch {
            appState?.pushNotice(AppNotice(tone: .error, message: String(format: String(localized: "Cache clear failed: %@"), error.localizedDescription)))
        }
        reload()
    }
}
