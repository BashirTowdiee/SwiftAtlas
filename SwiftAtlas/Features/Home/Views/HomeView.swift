  ...
  private var heroCard: some View {
    GSCard {
      VStack(alignment: .leading, spacing: GSSpacing.medium) {
        ...
        Text(
          "Every tab demonstrates a different layer of a serious SwiftUI app: app shell, design system, data flow, caching, memory ownership, and concurrency."
        )
    ...
    GSCard {
      VStack(alignment: .leading, spacing: GSSpacing.medium) {
        GSSectionHeader(
          ...
          detail:
            "The project is organized so the folder tree mirrors the ideas you are trying to learn."
        )
        ForEach(
          [
    ...
    GSCard {
      VStack(alignment: .leading, spacing: GSSpacing.medium) {
        GSSectionHeader(
          ...
          detail:
            "These are deliberately opinionated so the code reads like a standard, not a sketch."
        )
  ...
  private var diagnosticsCard: some View {
    GSCard {
      VStack(alignment: .leading, spacing: GSSpacing.medium) {
        ...
        GSInfoRow(
          title: "Reachability", value: container.appState.reachability.rawValue.capitalized)
        ...
        if let diagnostics = viewModel.diagnostics {
          ...
          GSInfoRow(
            title: "Version",
            value: "\(diagnostics.metadata.shortVersion) (\(diagnostics.metadata.buildNumber))")
        }
      }
    }
  }
