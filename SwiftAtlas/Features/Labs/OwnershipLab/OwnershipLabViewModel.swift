...
final class OwnershipLabViewModel: ObservableObject {
    ...
    func runRetainCycleDemo() {
        ...
        retainCycleExample = RetainCycleExample(
            onTeacherDeinit: { [weak self] in Task { @MainActor in self?.logs.append("Teacher deallocated.") } },
            onSessionDeinit: { [weak self] in Task { @MainActor in self?.logs.append("Session deallocated.") } }
        )
    ...
    func runFixedCycleDemo() {
        ...
        fixedCycleExample = FixedRetainCycleExample(
            onTeacherDeinit: { [weak self] in Task { @MainActor in self?.logs.append("Safe teacher deallocated.") } },
            onSessionDeinit: { [weak self] in Task { @MainActor in self?.logs.append("Safe session deallocated.") } }
        )
    ...
    func runClosureCaptureDemo(useWeakCapture: Bool) {
        ...
        closureExample = ClosureCaptureExample(
            ...
            onDeinit: { [weak self] in Task { @MainActor in self?.logs.append("Closure owner deallocated.") } }
        )
        if useWeakCapture {
