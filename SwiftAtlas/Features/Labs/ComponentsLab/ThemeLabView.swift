      ...
      VStack(alignment: .leading, spacing: GSSpacing.large) {
        GSCard {
          VStack(alignment: .leading, spacing: GSSpacing.medium) {
            GSSectionHeader(
              eyebrow: "Theme", title: "Semantic token mapping",
              detail:
                "Feature views never hardcode palette values; they ask the theme layer for meaning."
            )
            GSInfoRow(title: "Current Override", value: appState.themeOption.title)
            LazyVGrid(
              columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: GSSpacing.small
            ) {
