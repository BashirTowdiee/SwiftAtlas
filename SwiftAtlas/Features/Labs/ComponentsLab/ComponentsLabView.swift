    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: GSSpacing.large) {
                GSSectionHeader(eyebrow: "Components", title: "Reusable surfaces", detail: "These components are intentionally simple so their composition remains visible.")
                GSCard {
                ...
                GSCard {
                    VStack(alignment: .leading, spacing: GSSpacing.medium) {
                        ...
                        GSErrorView(error: .network("The component accepts an `AppError` and optionally a retry action."), retry: nil)
                    }
                }
            }
