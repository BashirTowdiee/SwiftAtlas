      ...
      VStack(alignment: .leading, spacing: GSSpacing.large) {
        GSCard {
          VStack(alignment: .leading, spacing: GSSpacing.small) {
            GSSectionHeader(
              eyebrow: "Inheritance", title: "Override control",
              detail:
                "Classes are final by default in this repo. Inheritance appears only when it teaches something specific."
            )
        ...
        GSCard {
          VStack(alignment: .leading, spacing: GSSpacing.small) {
            ...
            Text(
              "• Use `class` members only when overriding is intentional; otherwise prefer `static`."
            )
          }
        }
      }
